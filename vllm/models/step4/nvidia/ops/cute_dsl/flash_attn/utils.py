...
from vllm.models.step4.nvidia.ops.cute_dsl.utils import convert_from_dlpack as _convert_from_dlpack


def _cutlass_dsl_version() -> tuple[int, ...]:
    try:
        return tuple(int(part) for part in metadata.version("nvidia-cutlass-dsl").split(".")[:3])
    except (metadata.PackageNotFoundError, ValueError):
        return ()


_ROUND_RN = "rn" if _cutlass_dsl_version() >= (4, 5) else nvvm.RoundingModeKind.RN
_ROUND_RM = "rm" if _cutlass_dsl_version() >= (4, 5) else nvvm.RoundingModeKind.RM
...
fma_packed_f32x2 = partial(cute.arch.fma_packed_f32x2, rnd=_ROUND_RN)
mul_packed_f32x2 = partial(cute.arch.mul_packed_f32x2, rnd=_ROUND_RN)
add_packed_f32x2 = partial(cute.arch.add_packed_f32x2, rnd=_ROUND_RN)
...
def get_smem_store_atom(
    ...
    num_matrices: cutlass.Constexpr[int] = 4,
) -> cute.CopyAtom:
...
def ex2_emulation_2(x: Float32, y: Float32, *, loc=None, ip=None) -> Tuple[Float32, Float32]:
    ...
    # We want to round down here, so that the fractional part is in [0, 1)
    xy_rounded = cute.arch.add_packed_f32x2(xy_clamped, (fp32_round_int, fp32_round_int), rnd=_ROUND_RM)
